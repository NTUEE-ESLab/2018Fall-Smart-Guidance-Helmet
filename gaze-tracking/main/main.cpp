...
int main(int argc, char *argv[]){
	...
	try{
		...
		case InputMode::CAMERA:
			...
			eyecams[0] = std::make_unique<eye_tracker::EyeCamera>(0);
			eyecams[1] = std::make_unique<eye_tracker::EyeCamera>(1);
#endif
			eye_model_updaters[0] = std::make_unique<eye_tracker::EyeModelUpdater>(focal_length, 5, 0.5);
		...
		case InputMode::CAMERA_MONO:
			//eyecams[0] = std::make_unique<eye_tracker::EyeCamera>(0);
			eyecams[0] = std::make_unique<eye_tracker::EyeCamera>("udpsrc port=5000 ! application/x-rtp,media=video,payload=26,clock-rate=90000,encoding-name=JPEG,framerate=30/1 ! rtpjpegdepay ! jpegdec ! videoconvert ! appsink", cv::CAP_GSTREAMER);
			eye_model_updaters[0] = std::make_unique<eye_tracker::EyeModelUpdater>(focal_length, 5, 0.5);
			camera_undistorters[0] = std::make_unique<eye_tracker::CameraUndistorter>(K, distCoeffs);
			window_names = { "Cam0" };
			file_stems = { "cam0" };
	...
	while (is_run) {
		...
		for (size_t cam = 0; cam < kCameraNums; cam++) {
			...
			if (cam == 0 && kVisualization) {
				...
				// 2D pupil
				if (is_pupil_found) {
					cv::ellipse(img_rgb_debug, rr_pf, cv::Vec3b(255, 0, 255), 1); //(255, 128, 0)=BLUE
				}

				// 3D eye ball
				if (eye_model_updaters[cam]->is_model_built()) {
					cv::putText(img_rgb_debug, "Reliability: " + std::to_string(ellipse_realiability), cv::Point(30, 440), cv::FONT_HERSHEY_SIMPLEX, 1.0, cv::Scalar(0, 128, 255), 1);  //(0, 128, 255)=ORANGE
					if (is_reliable) {
				...
				file.open("../../../eye-direction-UI/coordinate.txt", ios::out);
				if (file){
					if (rr_pf.center.x == 0)
						file << "-1 0 0 0";
					else
						file << static_cast<int>(rr_pf.center.x) << " 0 0 0";
					file.close();
				}
		...
		static int ss = 0;
		if (ss++ > 50) {
			std::cout << "pupil_center.x: " << rr_pf.center.x << std::endl;
