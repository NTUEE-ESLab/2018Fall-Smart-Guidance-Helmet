...
EyeCamera::EyeCamera(const std::string file_name, int apiPreference, bool is_flipped)
{
	init(file_name, apiPreference, is_flipped);
}
void EyeCamera::init(const std::string file_name, int apiPreference, bool is_flipped)
{
	is_flipped_ = is_flipped;
	std::cout << "EyeCamera: Open from source: " << file_name << std::endl;
	cap_.open(file_name, apiPreference);
	is_image_ = false;
}
